...
export default function DashboardPage() {
  ...
  const fetchAiSummary = async () => {
    if (stats.numberOfDebtors === 0) {
      setAiSummary({
        summary: t('aiSummary.noDataSummary'),
        riskAssessment: t('aiSummary.noDataRisk'),
        recommendations: t('aiSummary.noDataRecommendations')
      });
      return;
    }
    ...
    try {
      ...
      setAiSummary({
        summary: t('aiSummary.errorSummary'),
        riskAssessment: t('aiSummary.errorRisk'),
        recommendations: t('aiSummary.errorRecommendations')
      });
    } finally {
  ...
  }, [loadingDebtors, debtors]); // Re-fetch if debtors data changes
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="space-y-4">
          {loadingAiSummary || loadingDebtors ? (
            ...
            <>
              <div>
                <h3 className="font-semibold text-foreground">{t('aiSummary.summaryTitle')}</h3>
                <p className="text-sm text-muted-foreground">{aiSummary.summary}</p>
              </div>
              <div>
                <h3 className="font-semibold text-foreground">{t('aiSummary.riskAssessmentTitle')}</h3>
                <p className="text-sm text-muted-foreground">{aiSummary.riskAssessment}</p>
              </div>
              <div>
                <h3 className="font-semibold text-foreground">{t('aiSummary.recommendationsTitle')}</h3>
                <p className="text-sm text-muted-foreground">{aiSummary.recommendations}</p>
              </div>
            </>
          ) : (
            <p className="text-sm text-muted-foreground">{t('refreshSummaryButton')} to generate insights.</p>
          )}
        </CardContent>
      </Card>
    </div>
