...
export default function DashboardPage() {
  ...
  const tAISummary = useTranslations("DashboardPage.aiSummary");
  ...
  const totalDebtTitle = t("totalDebt");
  const totalDebtDesc = (utilization: string) => t("totalDebtDescription", { utilization });
  const totalDebtorsTitle = t("totalDebtors");
  const totalDebtorsDesc = t("totalDebtorsDescription");
  const avgDebtTitle = t("avgDebtPerDebtor");
  const avgDebtDesc = t("avgDebtPerDebtorDescription");
  const debtorsOverLimitTitle = t("debtorsOverLimit");
  const debtorsOverLimitDesc = t("debtorsOverLimitDescription");
  const aiOverviewTitle = t("aiDebtOverviewTitle");
  const aiOverviewDesc = t("aiDebtOverviewDescription");
  const refreshButtonText = t("refreshSummaryButton");
  const refreshingButtonText = t("refreshingSummaryButton");
  const aiSummaryTitle = tAISummary("summaryTitle");
  const aiRiskTitle = tAISummary("riskAssessmentTitle");
  const aiRecsTitle = tAISummary("recommendationsTitle");
  const aiNoDataSummary = tAISummary("noDataSummary");
  const aiNoDataRisk = tAISummary("noDataRisk");
  const aiNoDataRecs = tAISummary("noDataRecommendations");
  const aiErrorSummary = tAISummary("errorSummary");
  const aiErrorRisk = tAISummary("errorRisk");
  const aiErrorRecs = tAISummary("errorRecommendations");
  ...
  }, [loadingDebtors, debtors]); // Dependencies are correct, adding translated strings to deps is not needed
