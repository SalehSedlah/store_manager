...
export default function HomePage({ params }: { params: { locale: ReturnType<typeof useLocale> }}) {
  const { user, loading } = useAuth();
  const router = useRouter();
  const pathname = usePathname(); // For knowing current path if needed for complex logic

  useEffect(() => {
    if (!loading) {
      if (user) {
        // router.replace constructs locale-aware path
